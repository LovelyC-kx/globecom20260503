...
\OrbitALIF{}-SNN nevertheless drops by only $-0.126$\,dB
($21.886\!\to\!21.760$\,dB). Two design choices explain this
robustness:
\begin{itemize}
    \item \emph{Plane-local BN.} Under \textsc{FedBN}, the BN and
    \TDBN\ affine parameters and running statistics never enter the
    inter-plane average (Section~\ref{sec:sysmodel}). Each plane
    therefore keeps a normalizer matched to its own climatology, and
    no statistical bias is imported from planes that observe a
    different distribution.
    \item \emph{Sub-threshold invariance.} An \FQS-LIF\ neuron emits
    no spike when the membrane potential stays below the firing
    threshold. A small per-plane shift in input intensity that does
    not push the membrane across that threshold leaves the spike
    pattern -- and hence every downstream feature -- unchanged.
\end{itemize}

PlainUNet enjoys neither property. At $\alpha\!=\!0.1$ its
centralized-to-federated PSNR drops by $2.86$\,dB, almost entirely a
partition-difficulty effect: under this skew $30$ of the $50$
satellites hold only the five-tile floor, so any aggregation rule
must reconcile severely imbalanced per-satellite coverage. Without
\OrbitALIF{}'s multi-domain decomposition (\DFRB{} and \SHAM{}), a
vanilla U-Net cannot recover the missing coverage from co-orbiting
peers. The richer per-satellite representations of \OrbitALIF{}
narrow this gap.
