...
compositing or Synthetic Aperture Radar(SAR) multi-sensor fusion to reconstruct.
Meraner et al.~\cite{ref:meraner2020} showed that a deep residual network conditioned on co-registered SAR data could effectively reconstruct cloud regions
, setting a strong baseline for subsequent fusion
methods. Ebel et al.~\cite{ref:ebel2021multisensor} further extended
this line of work to global, all-season settings, but they require
accurate registration and are therefore less suitable for time-critical scenarios where only a single
cloudy snapshot is available. To overcome this limitation, single-image
deep learning methods have been proposed. GAN- and CNN-based methods
such as SpA-GAN, AMGAN-CR, and MemoryNet directly learn a mapping from
a cloudy tile to its cloud-free counterpart~\cite{ref:cuhkcr}. More
recent transformer- and diffusion-based models further improve
restoration fidelity through long-range attention and iterative denoising~\cite{ref:cr-diffsurvey,ref:cr-cvpr2025,ref:cuhkcr}. Although these models provide strong restoration quality, their computational cost remains prohibitive for onboard satellite deployment. 


A way to reduce response latency is to move cloud
removal  to the satellite constellation,avoids downlinking large volumes of
cloud-contaminated imagery . However,
onboard cloud removal must satisfy several coupled physical
constraints. First, small and medium LEO satellites have tight power
budgets, typically on the order of $100$--$300$\,W for the whole
bus~\cite{ref:spacecpn1}]. Second, satellite-to-ground and
inter-satellite links have limited bandwidth and intermittent contact
windows~\cite{ref:spacecpn1}. Third, constellations have
asymmetric connectivity: intra-plane links are relatively stable,
whereas inter-plane links are available only under restricted orbital
visibility conditions~\cite{ref:walker1984}. Fourth, different
satellites observe different geographic regions, cloud climatologies,
illumination conditions, and sensor statistics, causing source-level
feature heterogeneity .

Federated learning (FL) provides a solution by letting each satellite train locally and share only model parameters. Early satellite FL work applied FedAvg directly to satellite--ground scenarios, designing visibility-aware schedulers and asynchronous aggregation to cope with intermittent downlinks~\cite{ref:fedavg,ref:razmi2022,ref:matthiesen2024}. The second generation moved aggregation into the constellation: FedMega~\cite{ref:fedmega2024} introduced ring all-reduce over intra-orbit ISLs to cut ground-link dependence, while hierarchical FL frameworks further relayed orbit-level models via MEO/GEO relays~\cite{ref:wu2023onboard,ref:wang2023intraplane}. The third generation removed the ground server entirely: DFedSat~\cite{ref:dfedsat} combines intra-orbit reduce with inter-orbit gossip over dynamic ISLs, enabling fully serverless cross-orbit training. In parallel, the task scope has expanded from land-cover classification and ship detection toward denser perception tasks such as semantic segmentation~\cite{ref:fedseg2025} and personalized super-resolution~\cite{ref:alanine2024} — yet dense pixel-level cloud removal, which demands per-pixel regression under physical non-IID, remains entirely unaddressed.
...
when no spike is emitted~\cite{ref:maass1997,ref:loihi}. Recent
low-level vision work has explored SNNs for image restoration tasks
such as deraining~\cite{ref:esdnet}. However, conventional binary
spikes provide limited activation expressiveness, which is
problematic for dense pixel-level regression. Aggressively reducing
computation can degrade restoration quality, while preserving quality
often requires deeper networks that weaken the energy advantage.
Therefore, onboard cloud removal requires a co-design of the neuron
activation, restoration backbone, and constellation-level aggregation
protocol.
...
\item We design a novel lightweight SNN backbone which analyze cloud-contaminated
imagery jointly across multiple domains of temporal,
spectral, and spatial to extract multi-dimensional fea-
ture representations. It comprises four specific modules and the complete
model contains only $2.30$\,M parameters.

\item We evaluate ORBITALIF on the public CUHK-CR cloud-
removal benchmark and a 5×10 Walker-Star constel-
lation . Under hierarchical inter-satellite links,
the federated ORBITALIF achieves performance com-
parable to an ANNUnet model of the same architecture with 72.3x reduce of energy. Under source-level Dirichlet heterogeneity with $\alpha\!=\!0.1$ and 0.01, our model is confirmed to be robust under extreme non-IID conditions,demonstrating the promise of such model.
\end{itemize}
...
\begin{itemize}
    \item  LoS visibility: For arbitrary satellite pair $s_{i_1,j_1}$ and $s_{i_2,j_2}$ (hereafter $s_1$ and $s_2$ if no ambiguity arises.) at altitude of $h_1$ and $h_2$. Their horizon half-angle $\rho_1,\rho_2$ is defined by $\rho_k = \arccos\left(\frac{r_E}{r_E + h_k}\right), k\in\{1,2\}$, which defines the cone within which it can see other satellites. Furthermore, denote their latitude by  $\theta_{1}^{v},\theta_{2}^{v}$ and longitude by $\theta_{1}^{h},\theta_{2}^{h}$, the geocentric angle can be written by:
    \begin{equation*}
          \varphi = \arccos\big[\sin\theta_{1}^{v} \sin\theta_{2}^{v} + \cos\theta_{1}^{v} \cos\theta_{2}^{v} \cos(\theta_{1}^{h} - \theta_{2}^{h})\big].  
    \end{equation*}
The two satellites are mutually LoS visible if and only if their geocentric angle does not exceed the sum of their individual horizon half-angle, i.e.,
$\varphi \leq\rho_1 + \rho_2$.
\item Doppler reliability: The Doppler shift captures the dynamic aspect of ISL reliability — even if two satellites have a clear line of sight, a high relative velocity can render the link unusable. The Doppler shift between satellites $s_1$ and $s_2$ is given by
$f_d = \frac{\psi\cdot f_c}{c}$,
where $\psi$ is the relative speed between the two satellites, $f_c$ is the carrier frequency, and $c$ is the speed of light. The two satellites are Doppler reliable if and only if this shift is stay within the tolerable threshold $f_{\max}$, i.e.,$f_d \leq f_{\max}$.
\end{itemize}
...
\end{aligned}
\end{equation*}
where $\mathbf{x}$ denotes the model parameters and $\ell(\cdot; \mathbf{z})$ is the loss on sample $\mathbf{z}$.
The learning procedure at each global iteration $t$ proceeds in four steps:
Step 1 — Model Distribution. At the beginning of iteration $t$, each orbital plane $i$ has a local model copy $\mathbf{x}_i^t$. Within each plane, satellites perform an intra-plane broadcast via the stable ring topology: the satellite that participated in the previous inter-plane aggregation disseminates the model $\mathbf{x}_i^t$ to all other satellites $s_{i,j}$ in the same orbit using the reliable intra-plane ISLs. After this step, every satellite holds $\mathbf{x}_i^t$ as its initial model for the current round.
Step 2 — Local Training. Each satellite $s_{i,j}$ performs local stochastic gradient descent (SGD) on its own dataset $\mathcal{D}_{i,j}$ for $E$ local epochs:
\begin{equation*}
\mathbf{x}_{i,j}^{t+\frac{1}{2}} = \mathbf{x}_i^t - \eta \sum_{e=0}^{E-1} \nabla F_{i,j}(\mathbf{x}_{i,j}^{t,e}),
\end{equation*}
where $\eta$ is the learning rate, $\nabla F_{i,j}(\cdot)$ denotes the stochastic gradient, and $\mathbf{x}_{i,j}^{t,e}$ represents the model after $e$ local steps.
Step 3 — Intra-Plane Model Aggregation (Ring All-Reduce). Since satellites within the same orbital plane move in the same direction at nearly constant velocities, their relative positions are stable and the intra-plane ISLs form a natural ring topology. This allows efficient ring all-reduce for intra-plane aggregation. Specifically, the $K$ satellites in orbit $i$ organize as a logical ring; each satellite sends its local update to its successor and receives an update from its predecessor. After $\mathcal{O}(K)$ communication rounds, every satellite in the plane obtains the exact average of all local models within that orbit:
\begin{equation*}
\mathbf{x}_i^{t+\frac{1}{2}} = \frac{1}{K} \sum_{j=1}^{K} \mathbf{x}_{i,j}^{t+\frac{1}{2}}, \quad \forall i \in \{1, \dots, N\}.
\end{equation*}
At this point, each orbit $i$ has a unified intra-plane model $\mathbf{x}_i^{t+\frac{1}{2}}$.
Step 4 — Inter-Plane Model Aggregation (Gossip Averaging). Unlike the stable intra-plane ring topology, inter-plane ISLs are unstable due to varying inclinations, altitudes, and relative velocities. Global aggregation across all $N$ orbits in a single round is impractical. Instead, a gossip averaging scheme is adopted: each orbital plane $i$ only communicates with its immediate neighbors on a pre-defined inter-plane connectivity graph $\mathcal{G} = (\mathcal{V}, \mathcal{E})$, where $\mathcal{V} = \{1, \dots, N\}$ and $e_{i,i'} \in \mathcal{E}$ only if there exists at least one credible pair between orbits $i$ and $i'$ (i.e., satisfying both LoS visibility and Doppler reliability constraints).
In each gossip round, orbit $i$ exchanges its model $\mathbf{x}_i^{t+\frac{1}{2}}$ with its neighbors $\mathcal{N}(i) = \{i' \mid e_{i,i'} \in \mathcal{E}\}$ and updates:
\begin{equation*}
\mathbf{x}_i^{t+1} = (1 - \alpha) \mathbf{x}_i^{t+\frac{1}{2}} + \alpha \cdot \frac{1}{|\mathcal{N}(i)|} \sum_{i' \in \mathcal{N}(i)} \mathbf{x}_{i'}^{t+\frac{1}{2}}
\end{equation*}
...
We use SNN model as the foundation and the most significant innovation in the architecture is
multi-domain decomposition ,cloud iamge behave differently across pixel, Fourier,
and time domains.
...
\textbf{Multi-domain decomposition.} Cloud artefacts behave
differently across representations: smooth blobs in the pixel domain,
mid-band concentration in the Fourier amplitude spectrum, and slow
variation along the spike time axis. Each module therefore decomposes
its input in its own native domain and fuses the components downstream.
...
the tile height and width in pixels, is embedded and replicated along a
synthetic time axis ($T\!=\!4$ steps). Stage~1 (highest resolution)
gets spectral and spatial information in both encoder and decoder,which is called SHAM; Stages~2 and~3
stack dual-frequency residual blocks with embedded \SHAM{} attention. Each skip
connection passes through an adaptive gate fusion module called AGFM before entering the decoder.
...
A binary LIF spike carries one bit information per step, forcing prior coding
schemes~\cite{ref:rueckauer2017,ref:kim2022burst} to use larger
steps to match ANN accuracy, but it is too costly for onboard deployment.
Deeper SNN restoration networks recover quality but erode the energy
advantage~\cite{ref:esdnet}. Extending the
output alphabet to five levels
$\{0,\tfrac{1}{4},\tfrac{1}{2},\tfrac{3}{4},1\}$, packing more
information into each spike so comparable quality is achieved at
$T\!=\!4$. This is called \FQS-LIF{}.The membrane update and spike output are
\begin{equation}
    u[t] \;=\; \bigl(u[t{-}1] - s[t{-}1]\bigr)\cdot\eta
              \;+\; \mathbf{W}\mathbf{x}[t], \qquad
%    s[t] \;=\; \frac{\mathrm{round}\!\bigl(
 %              \mathrm{clamp}(u[t],\,0,\,4)\bigr)}{4},
    \label{eq:5qs}
\end{equation}
where $u[t]\in\mathbb{R}$ is the membrane potential at step $t$,
$s[t{-}1]\in\{0,\tfrac{1}{4},\tfrac{1}{2},\tfrac{3}{4},1\}$ is the
spike output from the previous step, $\eta\!$ is the leak factor
controlling how fast the membrane decays between steps, $\mathbf{W}$ is
the synaptic weight matrix, and $\mathbf{x}[t]$ is the pre-synaptic
input at step $t$. The soft reset subtracts $s[t{-}1]$ rather than a
fixed threshold, carrying residual charge into the next step. Synaptic
operations are skipped when $s[t]\!=\!0$, preserving event-driven
sparsity. 
...
input feature tensor, $\mathbf{x}_h^{(1)}$ is the high-frequency spike
component, and $\mathbf{x}_l^{(1)}$ is the low-frequency residual. In spatial domain,
$\mathbf{x}_h^{(2)}$ and $\mathbf{x}_l^{(2)}$ are yeilding at reduced spatial
...
element-wise multiplication. Learnable parameter will be applied
to each branch before the final summation.
...
Standard U-Net connect encoder and decoder 
features directly. To avoid inconsistent feature aggregating.\AGFM{} introduces a learnable parameter as gate.
\noindent\textbf{Spectro-Temporal Spike Hyper-Block (\SSHB{}).}
Stage~1 operates at full $H{\times}W$ resolution and dominates the
compute budget. So we reduce spatial cost by trading pixels for time
steps ,
\begin{equation}
    [T,\,C,\,H,\,W]
...
Per-layer forward hooks record MAC count and
input non-zero rate.ANN energy uses
$4.6$\,pJ/MAC (Horowitz, 45\,nm CMOS~\cite{ref:horowitz}); SNN
energy uses $77$\,fJ/SOP for neuromorphic substrates
...
Different orbital planes observe different cloud climatologies.
We sweep Dirichlet $\alpha$ from $0.1$ to $0.01$ — a tenfold
tightening — and measure PSNR degradation. \OrbitALIF{}-SNN
degrades by only $-0.126$\,dB ($21.886\to21.760$\,dB). Keeping
both BN and \TDBN\ parameters local under \textsc{FedBN} prevents
normalization statistics from mixing across climatologically
distinct planes, absorbing the additional skew without re-tuning.
The $2.86$\,dB centralized-to-federated gap on PlainUNet reflects
the partition difficulty itself: under $\alpha\!=\!0.1$, 30 of 50
satellites hold only the five-tile floor, so any aggregation rule
faces severely imbalanced coverage. \OrbitALIF{} narrows this gap
through the richer per-satellite representations of \DFRB\ and
\SHAM.
