...
\newcommand{\OrbitANN}{\textsc{OrbitANN}}
\newcommand{\OrbitUnet}{\textsc{OrbitUnet}}
\newcommand{\FQS}{5QS}
\newcommand{\TDBN}{\textsc{TDBN}}
...
\textbf{\OrbitALIF{}} (ours) uses \FQS-LIF, \TDBN, $T\!=\!4$,
and base width $C\!=\!24$ ($2.30$\,M parameters).
\textbf{\OrbitANN{}} replaces every \FQS-LIF with ReLU and
every \TDBN\ with BN, keeping the same dataflow.
\textbf{\OrbitUnet{}} is a vanilla U-Net used as the aggregation
...
%\OrbitALIF{} alongside its ANN counterpart and the \OrbitUnet{}
...
$0.793$\,dB spiking cost is a worthwhile trade: \OrbitALIF{}
remains within $0.126$\,dB of the \OrbitUnet{} ANN baseline while
...
\midrule
\OrbitUnet{} (baseline)    & 22.012          & 0.6819          & 8{,}420  \\
\OrbitANN{}        & \textbf{22.679} & \textbf{0.6966} & 14{,}777 \\
\OrbitALIF{} (ours) & 21.886          & 0.6725          & 14{,}777 \\
\bottomrule
\end{tabular}
\end{table}
...
Under \textsc{FedBN}+Gossip with the full $5{\times}10$
constellation, \OrbitALIF{} drops by only $-0.126$\,dB
($21.886\!\to\!21.760$\,dB) -- a combined "model + plane-local BN"
effect, since BN and \TDBN\ statistics never enter the inter-plane
average (Section~\ref{sec:sysmodel}).

To probe the \emph{model-level} contribution alone -- without the
plane-local BN absorbing some of the skew -- we train the matched
ANN backbone (\OrbitANN{}, ReLU/BN drop-in) on a single satellite,
selecting at each $\alpha$ the satellite with the largest Dirichlet
slice and disabling all aggregation. Fig.~\ref{fig:alpha} reports
the resulting two PSNR-vs-round curves. Tightening $\alpha$ tenfold
shifts the \OrbitANN{} curve down by clearly more than the
$0.126$\,dB the federated \OrbitALIF{} loses across the same
$\alpha$ change, indicating that the spike-quantization gate of
\OrbitALIF{} contributes intrinsic robustness on top of the
federation-level normalization tricks.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig8_alpha_curves.png}
  \caption{\OrbitANN{} single-satellite PSNR vs.\ communication
  round on the largest Dirichlet slice at each $\alpha$, with no
  aggregation. Solid: $\alpha\!=\!0.1$; dashed: $\alpha\!=\!0.01$.
  The same backbone with \FQS-LIF\ activations (\OrbitALIF{}) loses
  only $0.126$\,dB across the same $\alpha$ change in the federated
  setting (Table~\ref{tab:federated}, $21.886\!\to\!21.760$\,dB).}
  \label{fig:alpha}
\end{figure}
...
slice of the data. \OrbitUnet{} -- the same \textsc{FedBN}+Gossip
...
min--max envelope of \OrbitALIF{} remains narrow throughout,
...
Under full data access, \OrbitANN{} reaches $25.374$\,dB on
CR1 and $23.197$\,dB on CR2, exceeding \OrbitUnet{} by $+0.502$\,dB
...
($-0.302$\,dB). B1 and B2 both fall \emph{below} \OrbitUnet{}
...
\OrbitUnet{} (C2)       & 1.31 & 24.872 & $-$0.502 & 0.7683 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!t]
  ...
  Columns: cloudy input / \OrbitALIF{} (ours) / \OrbitUnet{} /
...
$\alpha\!=\!0.1$, \OrbitALIF{} reaches $21.886$\,dB and
...
backbone outperforms \OrbitUnet{} by $+0.667$\,dB in the ANN variant,
