...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{fig_archi.png}
  \caption{Overview of \OrbitALIF{}. \textbf{Top-left:} single-satellite
  U-shaped backbone ($T\!=\!4$, $2.30$\,M parameters); stage~1 uses
  \SSHB{}, deeper stages stack \DFRB{} with embedded \SHAM{}
  attention, skip connections pass through \AGFM{} gates, and a global
  residual reconstructs the cloud-free image. \textbf{Top-right:} one
  global round in the life of satellite $s_{1,3}$ -- local SGD (S1),
  intra-plane mean over the $K\!=\!10$ sats including BN (S2),
  inter-plane chain-Gossip on the shared (non-BN) parameters (S3),
  and broadcast back to its 10 sats (S4). \textbf{Bottom:}
  inter-plane chain-Gossip on a $5$-plane chain -- each plane equally
  averages with its $1$--$2$ neighbours; BN parameters remain
  plane-local (FedBN).}
  \label{fig:archi}
\end{figure*}
