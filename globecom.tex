...
To probe the \emph{model-level} contribution alone, we additionally
train the matched ANN backbone (\OrbitANN{}, ReLU/BN drop-in)
on a single satellite -- the one holding the largest Dirichlet
slice at each $\alpha$ -- with no aggregation. Fig.~\ref{fig:alpha}
overlays the four PSNR-vs-round curves. Comparing the
$\alpha\!=\!0.1\!\to\!0.01$ gap of each backbone gives a clean
relative robustness reading: the ANN's gap is wider than the SNN's,
so the spike-quantization gate provides robustness independently of
the federation's normalization tricks.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{fig8_alpha_curves.png}
  \caption{Robustness check across $\alpha\!\in\!\{0.1,0.01\}$.
  Blue: \OrbitALIF{} under \textsc{FedBN}+Gossip on the full
  constellation. Orange: matched-architecture \OrbitANN{}
  trained on the single satellite holding the largest Dirichlet
  slice at each $\alpha$ (no aggregation). Solid: $\alpha\!=\!0.1$;
  dashed: $\alpha\!=\!0.01$. The model-level robustness gap (orange
  pair) is wider than the federation-included gap (blue pair).}
  \label{fig:alpha}
\end{figure}
