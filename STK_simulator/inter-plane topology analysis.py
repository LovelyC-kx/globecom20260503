...
def check_access(plane_idx, neighbor_idx, sat_dic):
    ...
    for i in range(len(connection_list)):
        ...
        while i < len(connection_list) and connection_list[i][0] <= flag_time:
            avg_snr = avg_snr * (connection_stop_time - flag_time) / (
                        connection_stop_time - flag_time + connection_list[i][1] - connection_list[i][0]) + \
                      connection_list[i][2] * (connection_list[i][1] - connection_list[i][0]) / (
                                  connection_stop_time - flag_time + connection_list[i][1] - connection_list[i][0])
            connection_stop_time = connection_list[i][1]
            # flag_snr = connection_list[i][2]
            i = i + 1
