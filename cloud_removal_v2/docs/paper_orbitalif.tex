...
\item \textbf{System results.} The $2.30$\,M onboard variant improves
  cloud removal by $+0.50$\,dB and reduces inference energy by up to
  $72.3\!\times$ over a compact ANN baseline. The \OrbitALIF{}-ANN
  backbone outperforms the PlainUNet baseline under the same
  decentralized FedBN\,+\,Gossip training protocol.
\end{itemize}
...
inter-plane traffic over shared (non-BN) parameters. The
\OrbitALIF{}-ANN row is pending and will be filled in the extended
version.}
\label{tab:federated}
\begin{tabular}{lcccc}
\toprule
...
\textbf{Federated results.}\;\;PlainUNet reaches $22.012$\,dB at
round 200 with $8.42$\,GB inter-plane traffic in $4.37$\,h, defining
the federated PSNR floor. \OrbitALIF{}-SNN attains
$21.886$\,dB\,/\,$0.6725$ with $14.78$\,GB traffic in $17.73$\,h:
within $0.13$\,dB of the plain ANN baseline despite operating with
sparse \FQS{}-LIF activations and TDBN-quantized intermediates. The
$2.86$\,dB centralized-vs-federated gap on PlainUNet quantifies the
cost of the $\alpha\!=\!0.1$ partition---$30/50$ satellites hold only
the floor of five tiles, so single satellites cannot reconstruct the
full encoder, and $R\!=\!200$ Gossip rounds only partially compensate.

\textbf{Heterogeneity sensitivity.}\;\;Increasing source-level
heterogeneity from $\alpha\!=\!0.1$ to $\alpha\!=\!0.01$ degrades
\OrbitALIF{}-SNN by only $-0.126$\,dB ($21.886\!\to\!21.760$\,dB,
$0.6725\!\to\!0.6691$ SSIM), confirming that \textsc{FedBN}-local
normalization absorbs most of the additional source skew without
re-tuning the protocol.
...
\OrbitALIF{}-SNN reaches $21.886$\,dB---within $0.13$\,dB of the
plain ANN federated baseline---and degrades by only $-0.126$\,dB
