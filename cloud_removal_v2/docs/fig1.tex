...
%   (a) OrbitVLIF architecture pipeline (single satellite)
%   (b) Decentralised federated topology over 5×10 LEO constellation
%
% Naming convention (DELIBERATELY DISTINCT from prior literature):
%   MFRB  = Multi-Frequency Residual Block   (replaces "SRB")
%   SHAM  = Spectral-Hybrid Attention Module (replaces "FSTA")
%   5QS   = 5-Level Quantized Spike          (replaces "MultiSpike-4")
%   CTM   = Channel-Time Modulation          (replaces "MDA")
%   AGFM  = Adaptive Gated Fusion Module     (replaces "GatedSkipFusion")
%   DSP   = Deep Spectral Processor          (replaces "L1 Block")
%
% Compile:
%   pdflatex fig1.tex
%
...
\definecolor{cBlue}  {HTML}{0173B2}     % OrbitVLIF / SNN
\definecolor{cOrange}{HTML}{DE8F05}     % ANN / Plain
\definecolor{cGreen} {HTML}{029E73}     % SHAM / accent 1
\definecolor{cRed}   {HTML}{D55E00}     % DSP / accent 2
\definecolor{cPurple}{HTML}{CC78BC}     % 5QS / accent 3
...
% =============================================================================
% Panel (a): OrbitVLIF architecture pipeline
% =============================================================================
...
% ---- Encoder (3 levels) -------------------------------------------------------
\node[encblk, minimum width=12mm] (e1) [right=3mm of pe] {MFRB$\times2$\\$\mathit{C}=24$};
\node[encblk, minimum width=12mm] (e2) [right=3mm of e1] {MFRB$\times2$\\$\mathit{C}=48$};
\node[encblk, minimum width=12mm] (e3) [right=3mm of e2] {MFRB$\times4$\\$\mathit{C}=96$};
...
\node[fusblk, fill=cRed!18, draw=cRed!75, minimum width=14mm, minimum height=6mm,
      ...
      (lvl1) [right=3mm of e3] {DSP\\($\,$5QS $+$ CTM$\,$)};
\draw[arrow] (e3) -- (lvl1);

% ---- Decoder (3 levels, mirrored, drawn below encoder) ------------------------
\node[decblk, minimum width=12mm] (d3) [below=8mm of e3]   {MFRB$\times2$};
\node[decblk, minimum width=12mm] (d2) [left=3mm of d3]    {MFRB$\times2$};
\node[decblk, minimum width=12mm] (d1) [left=3mm of d2]    {MFRB$\times2$};
...
\begin{scope}[every node/.style={font=\sffamily\scriptsize, align=center}]
  ...
  \node[block, fill=cPurple!18, draw=cPurple!70, minimum width=22mm, minimum height=5mm,
        font=\sffamily\scriptsize]
        (ms4) at ($(lvl1.north)+(0,9mm)$) {5QS quantizer\\$\{0,\tfrac14,\tfrac12,\tfrac34,1\}$};
  \draw[arrow, draw=cPurple!70, line width=0.4pt]
        (ms4.south) -- ($(lvl1.north)+(0,1pt)$);
\end{scope}

% ---- Panel (a) label ----------------------------------------------------------
\node[panellbl] at (-0.3, 1.8) {(a) OrbitVLIF architecture (single satellite)};
