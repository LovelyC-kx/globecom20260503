...
from STK_simulator.constellation_config import *
...
if __name__ == '__main__':
    ...
    #     num_satellites) + '_alpha_' + str(args2.plane_alpha) + '_auxi_' + str(7) + '_TrainSetPartition.pkl', 'wb') as f:
    #     pickle.dump(local_dataset_indices, f)

    with open(home_dir + 'Outputs/EuroSat_SNN_planes_' + str(num_planes) + '_satellites_' + str(
            num_satellites) + '_alpha_' + str(args2.plane_alpha) + '_auxi_' + str(7) + '_TrainSetPartition.pkl',
              'rb') as f:
    ...
    acc = numpy.zeros((2, repeat, args2.num_epoch))
    loss = numpy.zeros((2, repeat, args2.num_epoch))
    training_loss = numpy.zeros((2, repeat, args2.num_epoch))
    test_loss = numpy.zeros((2, repeat, args2.num_epoch))
    saved_acc = numpy.zeros((2, args2.num_epoch))
    saved_loss = numpy.zeros((2, args2.num_epoch))
    saved_training_loss = numpy.zeros((2, args2.num_epoch))
    saved_test_loss = numpy.zeros((2, args2.num_epoch))

    for r in range(repeat):
        # print('Optimized Training')
        # connectivity_matrix = WalkerDeltaConnectivityV2
        ...
        #
        # out_file_name = home_dir + 'Outputs/EuroSat_RoutingTree_Comparison_T_' + str(args2.T) + '_alpha_' + str(
        #     args2.plane_alpha) + '_num_planes_' + str(num_planes) + '_lr_' + str(args2.lr) + '_repeat_' + str(
    ...
    plot_results(acc, loss, args2.num_epoch, legends)