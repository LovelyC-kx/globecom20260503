...
def plot_spiking_rates(spiking_rates):
    ...
    bars = plt.bar(index, values, width=0.35, tick_label=x_ticks, color='#0072BD')
    for i in range(len(bars)):
        yval = values[i]
        plt.text(bars[i].get_x() + bars[i].get_width() / 2, yval, yval, ha='center', va='bottom', fontsize=15, color='#0072BD', weight='bold')
    ...
    image_name = home_dir + 'Outputs/EuroSat_Spiking_Rate_resnet.pdf'
...
def plot_energy_comparison(operations, spiking_rates, EMAC, EADD):
    ...
    for i in range(len(bars1)):
        ...
        plt.text(bars1[i].get_x() + bars1[i].get_width() / 2, yval, yval, ha='center', va='bottom', fontsize=15, color='#D95319', weight='bold')
    for i in range(len(bars2)):
        yval = round(energy_snn[i], 3)
        plt.text(bars2[i].get_x() + bars2[i].get_width() / 2, yval, yval, ha='center', va='bottom', fontsize=15, color='#EDB120', weight='bold')

    plt.xlabel('Layers', fontsize=25)
    plt.ylabel(r'Energy Consumed ($\mu J$)', fontsize=25)
    plt.xticks(index+bar_width/2, x_ticks, fontsize=18)
    plt.yticks(fontsize=20)
    plt.legend(fontsize=20, loc='upper left')
    plt.tight_layout()

    image_name = home_dir + 'Outputs/EuroSat_Energy_Comparison_resnet.pdf'
    fig.savefig(image_name, format='pdf', dpi=1200)
    plt.show()
...
if __name__ == '__main__':
    ...
    # spiking_rates = {'conv0.2': 0.8060, 'layer1.0.spike1': 0.6723, 'layer1.0.spike2': 0.7579, 'layer2.0.spike1': 0.3635, 'layer2.0.spike2': 0.5746, 'layer2.1.spike1': 0.3544, 'layer2.1.spike2': 0.6399, 'layer3.0.spike1': 0.3265, 'layer3.0.spike2': 0.5877, 'layer3.1.spike1': 0.3846, 'layer3.1.spike2': 1.0369}
    spiking_rates = {'conv0.2': 0.8060, 'layer1.0.1': 0.6723, 'layer1.0.2': 0.7579, 'layer2.0.1': 0.3635,
                     'layer2.0.2': 0.5746, 'layer2.1.1': 0.3544, 'layer2.1.2': 0.6399,
                     'layer3.0.1': 0.3265, 'layer3.0.2': 0.5877, 'layer3.1.1': 0.3846,
                     'layer3.1.2': 1.0369}
    plot_spiking_rates(spiking_rates)
    #
    # operations = {'conv1.2': 3 * 3 * 3 * 4 * 64 * 64, 'conv2.2': 3 * 3 * 4 * 8 * 32 * 32, 'fc.1': 8 * 16 * 16 * 32 }
    # operations = {'conv0.2': 3 * 3 * 3 * 16 * 64 * 64, 'layer1.0.spike1': 3 * 3 * 16 * 16 * 64 * 64, 'layer1.0.spike2': 3 * 3 * 16 * 16 * 64 * 64, 'layer2.0.spike1': 3 * 3 * 16 * 32 * 32 * 32, 'layer2.0.spike2': 3 * 3 * 32 * 32 * 32 * 32 + 1 * 1 * 16 * 32 * 32 * 32, 'layer2.1.spike1': 3 * 3 * 32 * 32 * 32 * 32, 'layer2.1.spike2': 3 * 3 * 32 * 32 * 32 * 32, 'layer3.0.spike1': 3 * 3 * 32 * 64 * 16 * 16, 'layer3.0.spike2': 3 * 3 * 64 * 64 * 16 * 16 + 1 * 1 * 32 * 64 * 16 * 16, 'layer3.1.spike1': 3 * 3 * 64 * 64 * 16 * 16, 'layer3.1.spike2': 3 * 3 * 64 * 64 * 16 * 16}
    operations = {'conv0.2': 3 * 3 * 3 * 16 * 64 * 64, 'layer1.0.1': 3 * 3 * 16 * 16 * 64 * 64,
                  'layer1.0.2': 3 * 3 * 16 * 16 * 64 * 64, 'layer2.0.1': 3 * 3 * 16 * 32 * 32 * 32,
                  'layer2.0.2': 3 * 3 * 32 * 32 * 32 * 32 + 1 * 1 * 16 * 32 * 32 * 32,
                  'layer2.1.1': 3 * 3 * 32 * 32 * 32 * 32, 'layer2.1.2': 3 * 3 * 32 * 32 * 32 * 32,
                  'layer3.0.1': 3 * 3 * 32 * 64 * 16 * 16,
                  'layer3.0.2': 3 * 3 * 64 * 64 * 16 * 16 + 1 * 1 * 32 * 64 * 16 * 16,
                  'layer3.1.1': 3 * 3 * 64 * 64 * 16 * 16, 'layer3.1.2': 3 * 3 * 64 * 64 * 16 * 16}
    EMAC = 4.6
    EADD = 0.9
